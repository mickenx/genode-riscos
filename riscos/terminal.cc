...
void Terminal::Main::_handle_input()
{
	...
	Genode::log("input char ",Char(codepoint.value));
	...
	Genode::log("input char ",Char(codepoint.value));
			...
			case CODEPOINT_F1:       special_sequence = "\EOP";Genode::log("F1");   break;
			...
			{
				Genode::log("readbuffer add");
				_read_buffer.add(codepoint);
				consume_character(codepoint);
			}
		});
	});
}
